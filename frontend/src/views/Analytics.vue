      ...
      <div v-else-if="campaignStats" class="campaign-stats">
        <div class="cs-header">
          <div>
            ...
            <span class="badge" :class="campaignStats.campaign?.status">{{
...
const stats = computed(() => [
  {
    ...
    value: overview.value.campaigns?.total || 0,
    icon: "📢",
    bg: "rgba(37,99,235,0.1)",
    sub: `${overview.value.campaigns?.active || 0} active`,
    color: "#2563eb",
  },
  {
    label: "Total Leads",
    value: overview.value.leads?.total || 0,
    icon: "👥",
    bg: "rgba(16,185,129,0.1)",
    sub: `${overview.value.leads?.converted || 0} converted`,
    color: "#10b981",
  },
  {
    label: "Emails Sent",
    value: overview.value.emails?.sent || 0,
    icon: "📧",
    bg: "rgba(124,58,237,0.1)",
    sub: `${overview.value.emails?.total || 0} total`,
    color: "#7c3aed",
  },
]);
...
const campaignStatsCards = computed(() => {
  ...
  return [
    ...
    },
    {
      ...
      value: l.new || 0,
      color: "#f59e0b",
      pct: `${Math.round(((l.new || 0) / total) * 100)}%`,
    },
    {
      label: "Converted",
      value: l.converted || 0,
      color: "#10b981",
      pct: `${Math.round(((l.converted || 0) / total) * 100)}%`,
    },
    {
      label: "Lost",
      value: l.lost || 0,
      color: "#ef4444",
      pct: `${Math.round(((l.lost || 0) / total) * 100)}%`,
    },
    {
      label: "Emails Sent",
      value: e.sent || 0,
      color: "#7c3aed",
      pct: `${Math.round(((e.sent || 0) / etotal) * 100)}%`,
    },
    {
      label: "Emails Failed",
      value: e.failed || 0,
      color: "#f43f5e",
      pct: `${Math.round(((e.failed || 0) / etotal) * 100)}%`,
    },
  ];
});
